...
function AppRoot() {
  ...
  return (
    <div className="mgt-canvas" style={{
      ...
      <AddStudentModal open={addStudent} onClose={() => setAddStudent(false)} onSave={() => {}}/>
      <AddPaymentModal open={addPayment.open} defaultStudentId={addPayment.studentId}
                       defaultAmount={addPayment.amount}
                       onClose={() => setAddPayment({ open: false, studentId: null, amount: null })} onSave={() => {}}/>
      <AddClassModal   open={addClass} onClose={() => setAddClass(false)} onSave={() => {}}/>
    </div>
  );
}
